...
function fetchAndCache (request, options) {
  ...
  if (navigator.userAgent.indexOf('Crosswalk') !== -1) {
    ...
    // console.log('crosswalk sw fetch credentials before fix', clonedRequest)
    if (clonedRequest.mode === 'cors' && clonedRequest.credentials === 'include') {
      clonedRequest = new Request(clonedRequest, {credentials: 'omit'})
      // console.log('crosswalk sw fetch credentials after fix', clonedRequest)
