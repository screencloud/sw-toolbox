...
Router.prototype.match = function(request, routeOnly) {
  var match = this.matchMethod(request.method, request.url) ||
      this.matchMethod('any', request.url);
  // hack - needed route only
  if (match === null) return null;
  if (routeOnly) return match[1]
  return match[0]()
};
