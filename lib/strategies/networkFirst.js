...
function networkFirst(request, values, options) {
  ...
  var offlineTimeoutSeconds = options.offlineTimeoutSeconds ||
      globalOptions.offlineTimeoutSeconds;
  if(offlineTimeoutSeconds && !navigator.onLine) {
    networkTimeoutSeconds = offlineTimeoutSeconds;
  }
  ...
  var matchRequest = new Request(url);
  ...
  return helpers.openCache(options).then(function(cache) {
    ...
    var networkPromise = helpers.fetchAndCache(request, options)
      .then(function(response) {
        ...
        return cache.match(matchRequest,
          cacheQueryOptions).then(function(response) {
